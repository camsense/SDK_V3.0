...
int main()
{
...
#ifdef _WIN32
    rtn = device.OpenSerial("//./com5", 115200);                       // For windows OS
#else
    ...
    while (true)
    {
        ...
        {
            printf("%d, dist: %d, angle: %f, is_invalid: %d, gray:%d\n",
                i, dataPack[i].dist, dataPack[i].angle, dataPack[i].flag, dataPack[i].syn_quality);
        }
        printf("\n");
    }
